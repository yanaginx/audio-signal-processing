...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    t1 = int(1.0/f1 * fs)
    t2 = int(1.0/f2 * fs)
    M = int(t1 * t2 / math.gcd(t1, t2))
    hM = int(M/2+1)
    X = fft(x[:M])
    mX = 20*np.log10(abs(X[:hM]))
    return mX


def check(output, expected_size, non_zero_indices):
    assert output.shape[0] == expected_size, "Test failed"
    def f1(o): return lambda idx: True if o[idx] > 0 else False
    filtered = filter(f1(output), non_zero_indices)
    assert len(list(filtered)) == len(
        non_zero_indices), f"Test failed with: size: {expected_size}"
...
# res_input = res['input']
# print(res['input'])
# minimized = minimizeEnergySpreadDFT(
#     res_input['x'], res_input['fs'], res_input['f1'], res_input['f2'])
# print(minimized)
# print(res['output'])
...
# # check(minimized, 126, [2, 5])
# check(minimized, 241, [3, 8])
