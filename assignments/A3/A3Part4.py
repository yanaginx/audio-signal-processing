...
def suppressFreqDFTmodel(x, fs, N):
    ...
    mX_filt = mX.copy()
    pX_filt = pX.copy()
    bin_width = int(np.ceil(70*M/fs))
    mX_filt[:bin_width+1] = -120
    y = dftSynth(mX, pX, M)*outputScaleFactor
    yfilt = dftSynth(mX_filt, pX_filt, M)*outputScaleFactor
...
# print(test_output[1])
# res = suppressFreqDFTmodel(test_input['x'], test_input['fs'], test_input['N'])
# print(res)
