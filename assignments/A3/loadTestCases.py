...
def load(partId, caseId=1):
    ...
    """
    try:
        data = pickle.load(open('testInput%s.pkl' % PA, 'rb'),
                           encoding='latin1')  # python3
    except TypeError:
        data = pickle.load(open('testInput%s.pkl' % PA, 'rb'))  # python2

    part = u'%s-part-%d' % (PA, partId)
    if part not in data['exampleInputs']:
        print("Please provide a valid partId (>=1), number of parts in this assignment are %d" % (
            len(data['exampleInputs'].keys())))
        return {'input': None, "output": None}
    if caseId > len(data['exampleInputs'][part]) or caseId <= 0:
        print("Please provide a valid caseId (>=1), number of test cases in this assignment are %d" % (
            len(data['exampleInputs'][part])))
        return {'input': None, "output": None}
