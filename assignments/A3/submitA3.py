...
def submitSolution_all_parts(email_adress, secret):
    ...
    for idx in range(len(LIST_PARTIDS)):
        ...
        if out == '':
            ...
            email.encoders.encode_base64(output_64_msg)
            # concatenating the source code for log
...
def source(partIdx):
    ...
    f = open(sourceFiles[partIdx],  encoding="utf-8")
    src = f.read()
    f.close()
...
def wrongOutputTypeError(outType, part):
    print("\n Type error in Part " + str(part+1) +
          " - The output data type of your function doesn't match the expected data type: (" + str(outType) + ").")
    # print "== Submission failed: Please correct and resubmit."
...
LIST_PARTIDS = ['H49Pi', 'SwB1b', 'Y3gtV',
                'nyZLq']  # CHANGE THE PART IDS HERE !!

# the "Identifier" you used when creating the part
partIds = ['A3-part-1', 'A3-part-2',
           'A3-part-3', 'A3-part-4']  # , 'A3-part-5']

# used to generate readable run-time information for students
partFriendlyNames = ['Minimize energy spread in DFT of sinusoids', 'Optimal zero-padding', 'Symmetry properties of the DFT',
                     'Suppressing frequency components using DFT model']  # , 'FFT size and zero padding (Optional)']
# source files to collect (just for our records)
sourceFiles = ['A3Part1.py', 'A3Part2.py',
               'A3Part3.py', 'A3Part4.py']  # , 'A3Part5.py']
