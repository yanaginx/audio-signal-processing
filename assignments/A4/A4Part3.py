...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    lowBandIdx3k = np.where(binFreq <= 3000)[0][-1]
    highBandIdx3k = np.where(binFreq > 3000)[0][0]
    highBandIdx10k = np.where(binFreq <= 10000)[0][-1]

    lowBandEnv_lin = np.sum(np.square(mXLin[:, lowBandIdx0:lowBandIdx3k+1]), 1)
    highBandEnv_lin = np.sum(
        np.square(mXLin[:, highBandIdx3k:highBandIdx10k+1]), 1)

    lowBandEnv = 10*np.log10(lowBandEnv_lin)
    highBandEnv = 10*np.log10(highBandEnv_lin)

    envelopes = np.zeros((numFrames, 2))
    envelopes[:, 0] = lowBandEnv
    envelopes[:, 1] = highBandEnv
    return envelopes


res = load(3, 1)
test = computeEngEnv(**res['input'])
print(test)
print(test.shape)
print(res['output'])
