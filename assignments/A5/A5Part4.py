...
def sineModelAnalEnhanced(inputFile='../../sounds/sines-440-602-transient.wav'):
    ...
    while pin < pend:                                        # while input sound pointer is within sound
        ...
        for pindex, p in enumerate(ploc):
            ...
            if p > 2 and p < (len(pX) - 2):
                # Select the peak if the phase spectrum around the peak is flat
                if selectFlatPhasePeak(pX, p, phaseDevThres):
                    plocSelMask[pindex] = 1
            else:
                plocSelMask[pindex] = 1
        # Select the ones chosen
        plocSel = ploc[plocSelMask.nonzero()[0]]
        # Ignoring frames that don't return two selected peaks
        if len(plocSel) != 2:
            ipfreq = [0.0, 0.0]
        else:
            # Only selected peaks to refine peak values by interpolation
            iploc, ipmag, ipphase = UF.peakInterp(mX, pX, plocSel)
    ...
    plt.pcolormesh(frmTime, binFreq, np.transpose(
...
test = load(4, 3)
res = selectFlatPhasePeak(**test['input'])
print(res)
print(test['output'])
